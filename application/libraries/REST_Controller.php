...
abstract class REST_Controller extends CI_Controller {
    ...
    public function __construct($config = 'rest')
    {
       ...
       header("Access-Control-Allow-Headers: X-API-KEY, Origin,token,isLogin,adminId,currentTime,techId,clientid,technicianID, X-Requested-With, Content-Type, Accept, Access-Control-Request-Method, Authorization");
    ...
    //             $this->config->item('rest_message_field_name') => $this->lang->line('text_rest_unknown_method'),
        ...
        if ($this->config->item('rest_enable_keys') && empty($this->rest->key) === FALSE)
        {
            ...
            {
                $response = [$this->config->item('rest_status_field_name') => FALSE, $this->config->item('rest_message_field_name') => $this->lang->line('text_rest_api_key_time_limit'),$this->config->item('rest_data_field_name') => null];
            ...
            if($authorized === FALSE)
            {
                ...
                $response = [$this->config->item('rest_status_field_name') => FALSE, $this->config->item('rest_message_field_name') => $this->lang->line('text_rest_api_key_permissions'),$this->config->item('rest_data_field_name') => null];
                $this->response($response, self::HTTP_UNAUTHORIZED);
        ...
        {
            $response = [$this->config->item('rest_status_field_name') => FALSE, $this->config->item('rest_message_field_name') => $this->lang->line('text_rest_ip_address_time_limit'),$this->config->item('rest_data_field_name') => null];
            $this->response($response, self::HTTP_UNAUTHORIZED);
    ...
    protected function _check_php_session()
    {
        ...
        if ( ! $this->session->userdata($key))
        {
            ...
            $this->response([
                $this->config->item('rest_status_field_name') => FALSE,
                $this->config->item('rest_message_field_name') => $this->lang->line('text_rest_unauthorized'),$this->config->item('rest_data_field_name') => null
            ], self::HTTP_UNAUTHORIZED);
        }
    }
